...
func newConnection() *grpc.ClientConn {
	path := "127.0.0.1:56666"
	conn, err := grpc.Dial(path, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	if err != nil {
		fmt.Printf("HealthCheck did not connect: %v", err)
		return nil
	}
	return conn
}
...
func EventChannel() {
	...
	for {
		if stream == nil {
			if stream, err = c.EventChannel(ctx, &emptypb.Empty{}); err != nil {
				fmt.Println(err)
				time.Sleep(time.Second * 5)
				continue
			}
...
func HealthCheck() {
	...
	r, err := c.GetServerToken(ctx, &emptypb.Empty{})
	if err != nil {
		fmt.Printf("HealthCheck: %v", err)
		return
	}
...
func GetAllStockDetail() {
	...
	r, err := c.GetAllStockDetail(ctx, &emptypb.Empty{})
	if err != nil {
		fmt.Printf("GetAllStockDetail: %v", err)
		return
	}
	for _, v := range r.GetStock() {
		fmt.Println(v)
	}
}
