...
func (r *SystemRepo) ActivateUser(ctx context.Context, username string) error {
	builder := r.Builder.Update(tableNameSystemAccount).
		Set("activated", true).
		Where("username = ?", username)
...
func (r *SystemRepo) QueryUserByUsername(ctx context.Context, username string) (*entity.User, error) {
	sql, arg, err := r.Builder.
		Select("username, password, email, email_verified, activated").
	...
	e := entity.User{}
	if err := row.Scan(&e.Username, &e.Password, &e.Email, &e.EmailVerified, &e.Activated); err != nil {
